  ...
  def apply(doc: D) = {
    ...
    val freq = sentences.foldLeft((Map[String, Int](), Map[String, Int]()))((m, sent) =>
      (m._1 ++
        ...
        sent.combinations(2).map({w => w.mkString(" ") -> (1 + m._1.getOrElse(w.mkString(" "), 0))})
      ) ->
      (m._2 ++
        sent.map({w => w -> (sent.length + m._2.getOrElse(w, 0))}) ++
        sent.combinations(2).map({w => w.mkString(" ") -> (sent.length + m._2.getOrElse(w.mkString(" "), 0))})
      )
    )

    val termF = freq._1.filter(_._2 > 3)
    val sentF = freq._2

    val allFreqTerms = termF.toSeq.sortWith((a, b) => a._2 > b._2 )
    val freqTerms = allFreqTerms.take(scala.math.floor(allFreqTerms.size.toDouble * 0.3).toInt).map(_._1)
